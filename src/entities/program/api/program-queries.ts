...
export async function listRecommendedPrograms(
  ...
  // NULL means the member skipped equipment setup, so do not pretend we know
  // what they can use. Popular published programs are the least surprising
  // recommendation when the filter is intentionally off.
  if (equipment === null) return data.slice(0, limit);
